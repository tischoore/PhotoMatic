...
| `src/nav_tree.rs` | Builds the Left Navigation tree's data (one node per top-level directory, with a count per currently-enabled File Type) from `list_directories()`/`directory_type_counts()` output plus the project's `FileExtensions` selection — pure logic, unit-tested independently of NWG and SQLite. |
