...
| `src/app.rs` | Main window, menu bar, three-region layout, Source Directory and File Types controls, the Left Navigation tree's context menu (Open in Explorer / Image List), Context Window tab management (open-or-switch, close via Ctrl+W or a right-click tab context menu), and event routing. |
...
| `src/exif.rs` | `read_metadata()`: reads an image's EXIF data (date taken, width, height, exposure time, ISO, focal length) off disk into an `ImageMetadata` — pure file I/O, no knowledge of the GUI or the database; unreadable files or files with no EXIF simply come back all-`None`. |
| `src/db/mod.rs` | `ProjectDb`: opens/migrates the project's SQLite database and folds each `ScanUnit` into it as the scan discovers it (`apply_scan_unit`), stamping `last_scan` once the scan finishes (`finish_scan`); also lists images still missing EXIF metadata (`list_images_pending_metadata`), writes it back (`update_image_metadata`), and lists images by directory/type for Context Window tabs (`list_images_by_directory`). |
| `src/db/migrations.rs` | The embedded migration list and `apply()`, run via `rusqlite_migration` against `PRAGMA user_version`. |
| `src/db/models.rs` | `ProjectSettings`, `ImageRecord` (including its `toplevel_dir` foreign key and EXIF metadata columns, plus `metadata_read_at` marking whether extraction has been attempted), `DirectoryRecord` — plain structs mirroring the database's tables. |
| `src/db/project_settings.rs` | Get/update queries for the singleton `project_settings` row. |
| `src/db/images.rs` | Upsert/list queries for the `images` table (including its EXIF metadata columns), the pending-metadata selection query, the metadata update query, `list_images_by_directory()` (by top-level directory and optional File Type, for Context Window tabs), and the pure `image_key()` (xxh3) hash function. |
...
| `src/context_tabs.rs` | Pure Context Window tab logic: `tab_title()` (directory or `"dir/type"` tab name) and `image_row()` (an `ImageRecord` as the 7 display strings shown in an Image List tab's table) — unit-tested independently of NWG and SQLite. |
