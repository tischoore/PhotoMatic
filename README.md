...
- **Left Navigation directory tree** — filling the entire Left Navigation panel, a tree view lists every top-level directory from the project database, each expandable into `jpg (n)`, `CR2 (n)`, and `gif (n)` child items showing that directory's image counts (0 when a type is absent). It's read entirely from the database and rebuilt whenever the database changes under it: when a project with an existing database finishes loading, and after every Scan Directory run. Right-clicking a directory node shows a one-item context menu, **Open in Explorer**, which opens Windows Explorer at that folder.
...
| `src/db/mod.rs` | `ProjectDb`: opens/migrates the project's SQLite database and folds each `ScanUnit` into it as the scan discovers it (`apply_scan_unit`), stamping `last_scan` once the scan finishes (`finish_scan`); also lists images still missing EXIF metadata (`list_images_pending_metadata`) and writes it back (`update_image_metadata`). |
...
| `src/db/images.rs` | Upsert/list queries for the `images` table (including its EXIF metadata columns), the pending-metadata selection query, the metadata update query, and the pure `image_key()` (xxh3) hash function. |
...
| `src/nav_tree.rs` | Builds the Left Navigation tree's data (one node per top-level directory, with fixed `jpg`/`cr2`/`gif` counts) from `list_directories()`/`directory_type_counts()` output — pure logic, unit-tested independently of NWG and SQLite. |
